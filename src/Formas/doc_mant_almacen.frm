...
Begin VB.Form doc_mant_almacen 
   ...
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_almacen.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_almacen.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_almacen.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_almacen.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   150
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_almacen"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT almac_codigo FROM docu_almacen WHERE almac_linea ='" & Cmb_linea.BoundText & "' and  almac_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_almacen (almac_linea, almac_codigo, almac_descripcion, almac_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "','A')"
     Else
        MsgBox "Este Almacen ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_almacen SET almac_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE almac_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and almac_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_almacen set almac_status ='E'" & _
             " WHERE almac_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and almac_codigo = '" & Trim$(txt_codigo.Text) & "'"
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "SELECT almac_linea as linea, almac_codigo as codigo, almac_descripcion as descripcion, almac_status as status" & _
                                " FROM docu_almacen ORDER BY almac_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_almacen set almac_status='A' " & _
                  "Where almac_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And almac_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("almac_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("almac_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("almac_descripcion"))
...
sql = "Select * FROM docu_almacen WHERE almac_linea ='" & Cmb_linea.BoundText & "' and almac_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_almacen WHERE almac_linea ='" & Cmb_linea.BoundText & "' and almac_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
