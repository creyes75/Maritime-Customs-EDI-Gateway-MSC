...
Begin VB.Form doc_mant_tipo_salida 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Tipo de Salida"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_tipo_salida.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_tipo_salida.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_tipo_salida.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_tipo_salida.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   180
            Width           =   465
      ...
      Begin Threed.SSFrame fra_tipo_rol 
         ...
         _ExtentY        =   3678
         _StockProps     =   14
         Caption         =   " Datos de Tipo de Salida "
         ForeColor       =   128
   ...
   End
End
Attribute VB_Name = "doc_mant_tipo_salida"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT tipo_sal_codigo FROM docu_tipo_salida WHERE tipo_sal_linea ='" & Cmb_linea.BoundText & "' and  tipo_sal_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_tipo_salida (tipo_sal_linea, tipo_sal_codigo, tipo_sal_descripcion, tipo_sal_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Este Tipo de Salida ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_tipo_salida SET tipo_sal_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE tipo_sal_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and tipo_sal_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_tipo_salida set tipo_sal_status = 'E' " & _
             " WHERE tipo_sal_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and tipo_sal_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
'    CReport.WindowTitle = App.Title
    Call Excel_repo_mant("tipo_salida")
    'CReport.Action = 1
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select tipo_sal_linea as linea, tipo_sal_codigo as codigo, tipo_sal_descripcion as descripcion, tipo_sal_status  as Status from docu_tipo_salida ORDER BY tipo_sal_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_tipo_salida set tipo_sal_status='A' " & _
                  "Where tipo_sal_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And tipo_sal_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("tipo_sal_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("tipo_sal_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("tipo_sal_descripcion"))
...
sql = "Select * FROM docu_tipo_salida WHERE tipo_sal_linea ='" & Cmb_linea.BoundText & "' and tipo_sal_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_tipo_salida WHERE tipo_sal_linea ='" & Cmb_linea.BoundText & "' and tipo_sal_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
