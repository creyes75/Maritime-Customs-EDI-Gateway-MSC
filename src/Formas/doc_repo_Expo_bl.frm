...
Begin VB.Form doc_repo_expo_bl 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Fra_pto_descarga 
         ...
         Begin VB.CommandButton Cmd_hlp_pto_emb 
            ...
            Height          =   300
            Left            =   1125
            Picture         =   "doc_repo_Expo_bl.frx":0000
            Style           =   1  'Graphical
            TabIndex        =   29
            ToolTipText     =   "Buscar"
            Top             =   255
      ...
      Begin VB.Frame Frame7 
         ...
         Top             =   1455
         Width           =   1500
         Begin MSDataListLib.DataCombo Cmb_pto_desc 
            Height          =   315
            Left            =   120
            TabIndex        =   3
            Top             =   240
      ...
      Begin VB.CommandButton Cmd_cons_bl 
         ...
         Height          =   675
         Left            =   4365
         Picture         =   "doc_repo_Expo_bl.frx":0532
         Style           =   1  'Graphical
         TabIndex        =   6
         Top             =   2700
         Width           =   990
   ...
   Begin VB.CommandButton Command2 
      Height          =   645
      Left            =   5085
      Picture         =   "doc_repo_Expo_bl.frx":222C
      Style           =   1  'Graphical
      TabIndex        =   8
      Top             =   6045
      Width           =   735
   ...
   Begin VB.CommandButton Command1 
      Height          =   645
      Left            =   4305
      Picture         =   "doc_repo_Expo_bl.frx":2536
      Style           =   1  'Graphical
      TabIndex        =   7
      Top             =   6045
      Width           =   735
   End
End
Attribute VB_Name = "doc_repo_expo_bl"
...
Dim st_arch_abierto As Boolean
...
Private Sub Cmb_pto_desc_Change()
...
If Opt_bl.Value Then
    sql = "Select blcab_cono_emb " & _
               ...
               "   and blcab_puer_des    ='" & Cmb_pto_desc.BoundText & "'"
...
Private Sub Cmb_pto_desc_KeyPress(KeyAscii As Integer)
If KeyAscii = 13 Then
SendKeys "{TAB}"
End If
End Sub
...
Private Sub Cmb_viaje_Change()
   ...
   carga_datacombo data_pto_emb, Cmb_pto_desc, sql, "mani_puer_des", "mani_puer_des"
...
Dim reg As New ADODB.Recordset
    ...
    sql = "Select blcab_cono_emb " & _
               ...
               "   and blcab_buque       ='" & Cmb_buque.BoundText & "'" & _
               "   and blcab_viaje       ='" & Cmb_viaje.BoundText & "'" & _
               "   and blcab_puer_des    ='" & Cmb_pto_desc.BoundText & "'" & _
...
'    If MsgBox("Este proceso tomará algunos minutos. Desea continuar?", vbQuestion + vbYesNo) = vbNo Then Exit Sub
    ...
    sql = "Select * " & _
               ...
               "   and blcab_buque       ='" & Cmb_buque.BoundText & "'" & _
               "   and blcab_viaje       ='" & Cmb_viaje.BoundText & "'"
               If Cmb_pto_desc.VisibleCount <> 0 Then
                  sql = Trim$(sql) & "   and blcab_puer_des    ='" & Trim(Cmb_pto_desc.BoundText) & "'"
               End If
               sql = Trim$(sql) & "   and blcab_cono_emb in (" & bls & ") " & _
                     " and blcab_puer_emb = '" & Gstr_param.pto_descarga & "'"
    ...
    '****************************************************************************
       ...
       oCell.Range.Text = Trim(reg("blcab_direc_consig"))
       ...
       oCell.Range.Text = Mid(reg("blcab_direc_embarc"), 61)
       ...
       'If reg("blcab_no_cont") = 0 Then
            oCell.Range.Text = IIf(Not IsNull(reg("blcab_bultos_tot")), reg("blcab_bultos_tot"), "0")
       'Else
       '     oCell.Range.Text = reg("blcab_no_cont")
       'End If
       
       sql = "Select count(*), tama_equip_abrev " & _
               " from docu_bldet_exp, docu_cont_exp, docu_tamano_equip " & _
               ...
               "   and tama_equip_linea =  bldet_linea  " & _
               "   and tama_equip_codigo = cont_tam_equip " & _
               " Group by tama_equip_abrev"
       REG_det.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
       Do While Not REG_det.EOF
            no_cont = no_cont & Chr(13) & REG_det(0) & "X" & REG_det(1)
            REG_det.MoveNext
       Loop
       REG_det.Close
       ...
       If Not REG_det.EOF Then
          oCell.Range.Text = reg("blcab_bultos_tot") & " " & REG_det("bldet_embalaje") & no_cont ' nuevo se agrega el embalaje    16/sep/2002
          For x = 1 To 10
              ...
              If Trim(REG_det("bldet_contenedor")) <> "" Then
                 ...
                 ls_sql = "Select * from docu_cont_exp " & _
                 ...
                 If Not reg_cont.EOF Then
                    marcas = marcas & Chr(13) & "     Seal :" & reg_cont("cont_sello1") & "     Type : " & devuelve_desc("docu_tamano_equip", "tama_equip_codigo", "tama_equip_descripcion", reg_cont("cont_tam_equip"), "tama_equip_linea = '" & Cmb_linea.BoundText & "'")
                 End If
                 If reg_cont("cont_condicion") = 3 Then
                    desc_carga = desc_carga & " (FCL/FCL) "
                    carrier1 = "SHIPPER'S LOAD, STOW AND COUNT"
                    carrier2 = "FCL/FCL"
                 End If
                 reg_cont.Close
              Else
              ...
              ' ----------  SEPT 22 2008  -----------------
              Set oCell = oWord.ActiveDocument.Tables(1).Cell(Row:=i, Column:=25 + x) ' desc. carga
              oCell.Range.Text = chk_str(Mid(desc_carga, 1, 125)) 'reg_det("bldet_desc_carg"))
              '------------------------------------------------------------------
              REG_det.MoveNext
          Next x
       End If
       ...
       Set oCell = oWord.ActiveDocument.Tables(1).Cell(Row:=i, Column:=60) 'carrier1
       oCell.Range.Text = carrier1
       Set oCell = oWord.ActiveDocument.Tables(1).Cell(Row:=i, Column:=61) 'carrier2
       oCell.Range.Text = carrier2
...
'                         reg_descrip.Close
                   ...
                   Else
                      marcas = chk_str(REG_det("bldet_marcas_num"))
                   End If
                   Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=linea, Column:=1) 'contenedor
                   oCell.Range.Text = marcas
                   Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=linea, Column:=2)  'sello
                   oCell.Range.Text = chk_str(REG_det("bldet_desc_carg"))
                   Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=linea, Column:=3)  'tipo
                   oCell.Range.Text = Format$(REG_det("bldet_PESO"), "#,###,###,##0.00")
                   Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=linea, Column:=4)  'PESO
                   oCell.Range.Text = Format$(REG_det("bldet_VOLUMEN"), "#,###,###,##0.00")
                   'Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=linea, Column:=5)  'NO BULTOS
...
Dim campo As String
    ...
    campo = "460": oWord.Selection.MoveDown Unit:=wdLine, Count:=90
...
'
    campo = "466": oWord.Selection.TypeText Text:=COD_BL
    ...
    campo = "471": oWord.Selection.ParagraphFormat.Alignment = wdAlignParagraphRight
    ...
    campo = "476": oWord.Selection.ParagraphFormat.Alignment = wdAlignParagraphLeft
    ...
    campo = "480": oWord.Selection.Font.Size = 8
    campo = "481": oWord.Selection.TypeParagraph
    campo = "494": oWord.Selection.MoveDown Unit:=wdLine, Count:=8
    campo = "481": oWord.Selection.TypeParagraph
    
    
    campo = "483": oWord.ActiveDocument.Tables.Add Range:=oWord.Selection.Range, NumRows:=1, NumColumns:=4
...
'    campo = "495": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=1, Column:=1)
'    campo = "496": oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10: oCell.Range.Text = "VESSEL :":
'    campo = "497": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=1, Column:=3)
'    campo = "498": oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10: oCell.Range.Text = "VOYAGE :":
'    campo = "499": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=2, Column:=1)
'    campo = "500": oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10: oCell.Range.Text = "PORT OF LOADING :":
'    campo = "501": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=2, Column:=3)
'    campo = "502": oCell.Range.Font.Bold = wdToggle:   oCell.Range.Font.Size = 10:  oCell.Range.Text = "PORT OF DISCHARGE :":
'    campo = "503": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=3, Column:=1)
'    campo = "504": oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10: oCell.Range.Text = "BILL OF LADING :":
    
'    campo = "506": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=1, Column:=2)
'    campo = "507": oCell.Range.Text = Trim$(Cmb_buque.Text)
'    campo = "508": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=1, Column:=4)
'    campo = "509": oCell.Range.Text = Trim$(Cmb_viaje.Text)
'    campo = "510": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=2, Column:=2)
'    campo = "511": oCell.Range.Text = Trim$(Cmb_pto_desc.Text) 'TL confirmar que puerto es
'    campo = "512": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=2, Column:=4)
'    campo = "513": oCell.Range.Text = "GUAYAQUIL"
'    campo = "514": Set oCell = oWord.ActiveDocument.Tables(4).Cell(Row:=3, Column:=2)
'    campo = "515": oCell.Range.Text = Trim$(COD_BL)
    ...
    campo = "520": oWord.Selection.TypeParagraph
    DoEvents
    campo = "460": oWord.Selection.MoveDown Unit:=wdLine, Count:=1
    
    ' se adiciona la otra tabla para los datos de los detalles
    campo = "521": oWord.ActiveDocument.Tables.Add Range:=oWord.Selection.Range, NumRows:=filas, NumColumns:=4
    ...
    campo = "460": oWord.Selection.MoveDown Unit:=wdLine, Count:=25
    
    oWord.ActiveDocument.Tables(5).Rows.SetLeftIndent LeftIndent:=-32.5, RulerStyle:=wdAdjustNone
    oWord.ActiveDocument.Tables(5).Columns(1).SetWidth ColumnWidth:=100, RulerStyle:=wdAdjustNone
    oWord.ActiveDocument.Tables(5).Columns(4).SetWidth ColumnWidth:=54, RulerStyle:=wdAdjustNone
    DoEvents
    
    oWord.ActiveDocument.Tables(5).Columns(3).SetWidth ColumnWidth:=67.5, RulerStyle:=wdAdjustNone
    oWord.ActiveDocument.Tables(5).Columns(2).SetWidth ColumnWidth:=327, RulerStyle:=wdAdjustNone
    DoEvents
'    campo = "532": Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=1, Column:=1)
'    campo = "533": oCell.Range.Text = "CONTEINER No.": oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10
'    campo = "534": Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=1, Column:=2)
'    campo = "535": oCell.Range.Text = "SEAL":          oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10
'    campo = "536": Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=1, Column:=3)
'    campo = "537": oCell.Range.Text = "TYPE":          oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10
'    campo = "538": Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=1, Column:=4)
'    campo = "539": oCell.Range.Text = "WEIGHT":        oCell.Range.Font.Bold = wdToggle:    oCell.Range.Font.Size = 10
    'campo = "540": Set oCell = oWord.ActiveDocument.Tables(5).Cell(Row:=1, Column:=5)
...
errores_anexo:
   ...
   Resume 0
End Function
Function anexo_contenedores_completo(filas As Integer, COD_BL As String)
On Error GoTo errores_anexo
Dim campo As String

    campo = "460": oWord.Selection.MoveDown Unit:=wdLine, Count:=100
    campo = "461": oWord.Selection.MoveRight Unit:=wdCharacter, Count:=1
    campo = "462": oWord.Selection.InsertBreak Type:=wdPageBreak
    campo = "463": oWord.Selection.TypeParagraph
    campo = "464": oWord.Selection.TypeParagraph
...
If Opt_bl.Value Then
    sql = "Select blcab_cono_emb " & _
               ...
               "   and blcab_buque       ='" & Cmb_buque.BoundText & "'" & _
               "   and blcab_viaje       ='" & Cmb_viaje.BoundText & "'" & _
               "   and blcab_puer_emb    ='" & Cmb_pto_desc.BoundText & "'" & _
