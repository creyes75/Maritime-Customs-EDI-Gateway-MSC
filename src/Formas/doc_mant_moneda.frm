...
Begin VB.Form doc_mant_moneda 
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_moneda.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_moneda.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_moneda.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   195
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_moneda"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT mone_codigo FROM docu_moneda WHERE mone_linea ='" & Cmb_linea.BoundText & "' and  mone_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_moneda (mone_linea, mone_codigo, mone_descripcion, mone_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Esta Moneda ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_moneda SET mone_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE mone_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and mone_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_moneda set mone_status = 'E' " & _
             " WHERE mone_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and mone_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select mone_linea as linea, mone_codigo as codigo, mone_descripcion as descripcion, mone_status  as Status from docu_moneda ORDER BY mone_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_moneda set mone_status='A' " & _
                  "Where mone_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And mone_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("mone_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("mone_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("mone_descripcion"))
...
sql = "Select * FROM docu_moneda WHERE mone_linea ='" & Cmb_linea.BoundText & "' and mone_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_moneda WHERE mone_linea ='" & Cmb_linea.BoundText & "' and mone_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
