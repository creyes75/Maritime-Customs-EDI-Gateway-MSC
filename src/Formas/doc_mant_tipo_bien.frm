...
Begin VB.Form doc_mant_tipo_bien 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Tipo de Bien"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_tipo_bien.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_tipo_bien.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_tipo_bien.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_tipo_bien.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   180
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_tipo_bien"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT tipo_bien_codigo FROM docu_tipo_bien WHERE tipo_bien_linea ='" & Cmb_linea.BoundText & "' and  tipo_bien_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_tipo_bien (tipo_bien_linea, tipo_bien_codigo, tipo_bien_descripcion, tipo_bien_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Esta Tipo de Bien ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_tipo_bien SET tipo_bien_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE tipo_bien_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and tipo_bien_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_tipo_bien set tipo_bien_status = 'E' " & _
             " WHERE tipo_bien_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and tipo_bien_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
'    CReport.WindowTitle = App.Title
    'CReport.Action = 1
    Call Excel_repo_mant("tipo_bien")
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
End Sub
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select tipo_bien_linea as linea, tipo_bien_codigo as codigo, tipo_bien_descripcion as descripcion, tipo_bien_status  as Status from docu_tipo_bien ORDER BY tipo_bien_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_tipo_bien set tipo_bien_status='A' " & _
                  "Where tipo_bien_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And tipo_bien_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("tipo_bien_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("tipo_bien_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("tipo_bien_descripcion"))
...
sql = "Select * FROM docu_tipo_bien WHERE tipo_bien_linea ='" & Cmb_linea.BoundText & "' and tipo_bien_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_tipo_bien WHERE tipo_bien_linea ='" & Cmb_linea.BoundText & "' and tipo_bien_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
