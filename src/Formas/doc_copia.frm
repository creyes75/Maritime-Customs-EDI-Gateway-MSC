...
Begin VB.Form Frm_copia 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Frame Fra_pto_descarga 
         ...
         Top             =   735
         Width           =   2205
         Begin MSAdodcLib.Adodc data_pto_emb_new 
            Height          =   330
            Left            =   1470
            Top             =   210
            Visible         =   0   'False
            ...
            _Version        =   393216
         End
         Begin MSDataListLib.DataCombo cmb_pto_emb_new 
            Height          =   315
            Left            =   105
            TabIndex        =   7
            Top             =   225
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Fra_buque 
         ...
         End
      End
      Begin MSDataListLib.DataCombo Cmb_Bl 
         Height          =   315
         Left            =   2550
         TabIndex        =   4
         Top             =   1020
         ...
         Text            =   ""
      End
      Begin MSAdodcLib.Adodc DATA_BL 
         Height          =   330
         Left            =   4545
         Top             =   1020
         Visible         =   0   'False
...
'Fecha        :  Martes 30 de Julio del 2002
'Proceso      :  Validar que la linea,buque,viaje,puerto,bl no exista ya en la tabla
'                docu_blcab, docu_bldet.
'                copiar el Bl en el linea,buque,viaje,puerto seleccionado con el mismo
...
If Not Cmb_buque.MatchedWithList Or Not Cmb_viaje.MatchedWithList Or _
   Not Cmb_Bl.MatchedWithList Or Not Cmb_pto_emb.MatchedWithList Then
   MsgBox "No ha escogido los dato origen de la copia", vbInformation
   Exit Sub
End If

If Not Cmb_buque_new.MatchedWithList Or Not Cmb_viaje_new.MatchedWithList Or _
   Len(Trim(txt_bl_new.Text)) = 0 Or Not cmb_pto_emb_new.MatchedWithList Then
...
puerto = Cmb_pto_emb.BoundText
bl = Trim$(Cmb_Bl.BoundText)

buque_new = Cmb_buque_new.BoundText
viaje_new = Cmb_viaje_new.BoundText
puerto_new = cmb_pto_emb_new.BoundText
bl_new = Trim$(txt_bl_new.Text)

' se valida que el bl destino no exista
   sql = "SELECT * FROM docu_blcab " & _
         " WHERE blcab_linea = '" & linea & "'" & _
         "   and blcab_buque = '" & buque_new & "'" & _
         "   and blcab_viaje = '" & viaje_new & "'" & _
         "   and blcab_puer_emb = '" & puerto_new & "'" & _
         "   and blcab_cono_emb = '" & bl_new & "'"

   reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
If Not reg.EOF Then
   MsgBox "El bl destino de la copia ya existe, por favor verifique", vbInformation
   Exit Sub
End If
...
If Not reg2.EOF Then
   ...
   sql = "INSERT INTO docu_blcab SELECT blcab_linea,   '" & buque_new & "', '" & viaje_new & "', '" & _
                                      puerto_new & "',      blcab_puer_des,     blcab_emp_resp, '" & _
                                      bl_new & "',          blcab_emp_anav,     blcab_tipo_doc, " & _
   ...
   sql = "insert into docu_bldet " & _
         "SELECT bldet_linea,   '" & buque_new & "', '" & viaje_new & "', '" & _
                 puerto_new & "',    bldet_puer_des,      bldet_emp_resp, '" & _
                 bl_new & "',        bldet_secuencia,     bldet_tipo_carga, " & _
               ...
               " bldet_tip_equi,     bldet_cond_cont " & _
         " FROM docu_bldet WHERE bldet_linea ='" & linea & "'"
   ...
   sql = "INSERT INTO docu_cont  " & _
         "SELECT cont_linea,    '" & buque_new & "', '" & viaje_new & "', '" & _
                 puerto_new & "',    cont_puer_des,      cont_contenedor,  " & _
                ...
                "cont_temp_max_manip "
   sql = sql & "  FROM docu_cont, docu_bldet " & _
   ...
   If Chk_borra_bl.Value = 1 Then
        sql = "Delete " & _
                    ...
                    "       Select bldet_contenedor FROM docu_bldet " & _
...
Else
    MsgBox "El bl Original No existe. Por favor verifique", vbInformation
End If
...
Private Sub Cmb_Bl_KeyPress(KeyAscii As Integer)
...
Private Sub Cmb_pto_emb_Change()
   ...
   sql = "Select * from docu_blcab " & _
         ...
         "   And blcab_puer_des ='" & Gstr_param.pto_descarga & "'" '& _
         " Order by blcab_cono_emb"
         

carga_datacombo DATA_BL, Cmb_Bl, sql, "blcab_cono_emb", "blcab_cono_emb"
...
Private Sub cmb_pto_emb_new_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then SendKeys "{TAB}"
End Sub
...
'carga_datacombo data_pto_emb_new, cmb_pto_emb_new, sql, "mani_puer_emb", "mani_puer_emb"
...
Private Sub txt_bl_new_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then SendKeys "{TAB}"
End Sub

Public Sub codigo_nousado()
...
'                                     " blcab_seguro,        blcab_moneda_seguro,blcab_metod_pago_otros," & _
'                                     " blcab_fecha_emb )" & _
'                "VALUES ('" & Cmb_linea.BoundText & "',    '" & Cmb_buque_new.BoundText & "',       '" & Cmb_viaje_new.BoundText & "', "
'           sql = sql & "'" & cmb_pto_emb_new.BoundText & "',       '" & reg2("blcab_puer_des") & "',     '" & reg2("blcab_emp_resp") & "', "
'           sql = sql & "'" & txt_bl_new.Text & "',            '" & reg2("blcab_emp_anav") & "',    '" & reg2("blcab_tipo_doc") & "', "
'           sql = Trim$(sql) & "'" & reg2("blcab_cono_master") & "',      " & reg2("blcab_peso_tot") & ",     " & reg2("blcab_bultos_tot") & ", " & _
'                        "'" & reg2("blcab_tipo_ide_cons") & "', '" & reg2("blcab_ide_cons") & "',  '" & reg2("blcab_consig") & "', " & _
'                        "'" & reg2("blcab_direc_consig") & "',        '" & reg2("blcab_nacio_consig") & "', '" & reg2("blcab_tipo_ide_notif") & "', " & _
'                        "'" & reg2("blcab_ide_notif") & "',             '" & reg2("blcab_notif") & "','" & reg2("blcab_direc_notif") & "', " & _
'                        "'" & reg2("blcab_embarcador") & "',          '" & reg2("blcab_direc_embarc") & "',  '" & reg2("blcab_puer_origen") & "', "
'
'           sql = sql & " '" & reg2("blcab_almac_temp") & "',          '" & reg2("blcab_tipo_depos") & "','" & reg2("blcab_deposito_adua") & "', " & _
'                        "'" & reg2("blcab_regimen_adua") & "',          '" & reg2("blcab_consolidad") & "',     " & _
'                        "'" & reg2("blcab_tipo_salida") & "','" & reg2("blcab_docum_aut") & "', " & _
'                        " " & reg2("blcab_pesob_mal_est") & ",                    " & reg2("blcab_bultos_mal_est") & ",        '" & reg2("blcab_notif2") & "', " & _
'                        "'" & reg2("blcab_direc_notif2") & "',          " & reg2("blcab_no_cont") & ",      " & reg2("blcab_volumen_tot") & ", " & _
'                        "'" & reg2("blcab_pto_dest_final") & "',                '" & reg2("blcab_metod_pago") & "',  '" & reg2("blcab_moneda") & "', " & _
'                        " " & reg2("blcab_flete") & ",                 " & reg2("blcab_otros_gastos") & ", '" & reg2("blcab_moneda_otros") & "', " & _
'                        " " & reg2("blcab_seguro") & ",               '" & reg2("blcab_moneda_seguro") & "','" & reg2("blcab_metod_pago_otros") & "', " & _
'                        "'" & reg2("blcab_fecha_emb") & "')"
'   reg2.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'
'   Do While Not reg2.EOF
'      sql = "insert into docu_bldet " & _
'            " VALUES ('" & Cmb_linea.BoundText & "',    '" & Cmb_buque.BoundText & "',      '" & Cmb_viaje.BoundText & "', "
'           If hay_puerto = 1 Then
'              sql = Trim$(sql) & "'" & cmb_pto_emb_new.BoundText & "',       '" & reg2("bldet_puer_des") & "',     '" & reg2("bldet_emp_resp") & "', "
'           Else
'              sql = Trim$(sql) & "'" & Cmb_pto_emb.BoundText & "',       '" & reg2("bldet_puer_des") & "',     '" & reg2("bldet_emp_resp") & "', "
'           End If
'           If hay_bl = 1 Then
'              sql = Trim$(sql) & "'" & txt_bl_new.Text & "',             " & reg2("bldet_secuencia") & ",                          '" & reg2("bldet_tipo_carga") & "', "
'           Else
'              sql = Trim$(sql) & "'" & txt_bl.Text & "',             " & reg2("bldet_secuencia") & ",                          '" & reg2("bldet_tipo_carga") & "', "
'           End If
'           sql = Trim$(sql) & " " & reg2("bldet_peso") & ",              " & reg2("bldet_bultos") & ",          '" & reg2("bldet_embalaje") & "', " & _
'                  "'" & reg2("bldet_marcas_num") & "', '" & reg2("bldet_desc_carg") & "',   " & reg2("bldet_volumen") & ", " & _
'                  "'" & reg2("bldet_contenedor") & "','" & reg2("bldet_just_cambio") & "', '" & reg2("bldet_IMO") & "'," & _
'                  "'" & reg2("bldet_pelig_ONU") & "', '" & reg2("bldet_sello") & "',       '" & reg2("bldet_tam_equi") & "'," & _
'                  "'" & reg2("bldet_tip_equi") & "', '" & reg2("bldet_cond_cont") & "')"
'
'      Gcon_docu.Execute sql, no_det
'      reg2.MoveNext
'   Loop
...
'   docu_cont.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'
'   Do While Not reg2.EOF
'        ' para validar si existe ya el contenedor en el nuevo puerto
'        sql = "SELECT * FROM docu_cont WHERE cont_linea ='" & linea & "'"
'        sql = sql + " and cont_buque = '" & buque & "'"
'        sql = sql + " and cont_viaje = '" & viaje & "'"
'        sql = sql + " and cont_puer_emb = '" & cmb_pto_emb_new.BoundText & "'"
'        sql = sql + " and cont_puer_des = '" & puerto_des & "'"
'        sql = sql + " and cont_contenedor = '" & reg2("cont_contenedor") & "'"
'
'        reg3.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'        If reg3.EOF Then
'           sql = "insert into docu_cont " & _
'                 " VALUES ('" & Cmb_linea.BoundText & "',    '" & Cmb_buque.BoundText & "',      '" & Cmb_viaje.BoundText & "', "
'                 If hay_puerto = 1 Then
'                    sql = Trim$(sql) & "'" & cmb_pto_emb_new.BoundText & "',       '" & reg2("cont_contenedor") & "',     '" & reg2("cont_tam_equip") & "', "
'                 Else
'                    sql = Trim$(sql) & "'" & Cmb_pto_emb.BoundText & "',       '" & reg2("cont_contenedor") & "',     '" & reg2("cont_tam_equip") & "', "
'           sql = Trim$(sql) & "'" & reg2("cont_condicion") & ",              " & reg2("cont_tara") & ",          '" & reg2("cont_tipo_equip") & "', " & _
'                  "'" & reg2("cont_proved_equip") & "', '" & reg2("cont_estad_equip") & "',   " & reg2("cont_indic_llen_vac") & ", " & _
'                  "'" & reg2("cont_cond_contrato") & "','" & reg2("cont_reg_serv_transp") & "', '" & reg2("cont_natu_carg") & "'," & _
'                  "'" & reg2("cont_peso") & "', '" & reg2("cont_tipo_bien") & "',       '" & reg2("cont_sello1") & "'," & _
'                  "'" & reg2("cont_sello2") & "', '" & reg2("cont_sello3") & "','" & reg3("cont_sello4") & "',  " & _
'                  "'" & reg2("cont_temp_dur_almac") & "', '" & reg2("cont_temp_dur_trans") & "','" & reg2("cont_temp_dur_manip") & "',  " & _
'                  "'" & reg2("cont_temp_nim_almac") & "', '" & reg2("cont_temp_max_almac") & "', '" & reg2("cont_temp_nim_trans") & "',  " & _
'                  "'" & reg2("cont_temp_max_trans") & "', '" & reg2("cont_temp_nim_manip") & "', '" & reg2("cont_temp_max_manip") & "')"
'
