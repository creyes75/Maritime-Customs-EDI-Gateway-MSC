   ...
   If Lstr_opc = "A" Then
     ...
     If reg.EOF Then
        sql = "INSERT INTO docu_buque_viaje (buqv_linea, buqv_buque, buqv_viaje, buqv_fecha_arribo, buqv_direccion, buqv_observacion, buqv_status, buqv_fecha_ing, buqv_user_ing)" & _
             " VALUES ('" & Trim$(Cmb_linea.BoundText) & "','" & Trim$(Cmb_vessel.BoundText) & "','" & Trim$(Txt_voyage.Text) & "','" & Format(GTD_date.Text, "dd/mm/yyyy") & "','" & Direction & "','" & Trim$(txt_desc.Text) & "', 'A', '" & Format$(Date, "dd/mm/yyyy") & "','" & Gstr_user & "')"
     Else
        MsgBox "Este Buque/Viaje ya existe, entre por la opción modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_buque_viaje SET buqv_fecha_arribo = '" & GTD_date.Text & "', " & _
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select buqv_linea as Linea, buqv_buque as Vessel, buqv_viaje as Voyage, buqv_direccion as Direction, buqv_fecha_arribo as SailArrivDate, buqv_observacion as Comments from docu_buque_viaje Where buqv_status = 'A' ORDER BY  Linea, Vessel, Voyage"
   RDC_grid_dato.Refresh
...
If Not RDC_dato.Recordset.EOF Then
   ...
   Txt_voyage.Text = Trim$(RDC_dato.Recordset.Fields("buqv_viaje"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("buqv_observacion"))
