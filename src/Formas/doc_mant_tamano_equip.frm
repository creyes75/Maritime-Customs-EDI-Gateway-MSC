...
Begin VB.Form doc_mant_tamano_equip 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Tamaño de Equipo (Tipo Contenedor)"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_tamano_equip.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_tamano_equip.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_tamano_equip.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_tamano_equip.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   16
            Top             =   180
            Width           =   465
      ...
      Begin Threed.SSFrame fra_tipo_rol 
         ...
         _ExtentY        =   3678
         _StockProps     =   14
         Caption         =   " Datos de Tamaño de Equipo "
         ForeColor       =   128
   ...
   End
End
Attribute VB_Name = "doc_mant_tamano_equip"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT tama_equip_codigo FROM docu_tamano_equip WHERE tama_equip_linea ='" & Cmb_linea.BoundText & "' and  tama_equip_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_tamano_equip (tama_equip_linea, tama_equip_codigo, tama_equip_descripcion, tama_equip_status, tama_equip_abrev)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "', '" & Trim$(txt_desc.Text) & "', 'A', '" & Txt_abrev.Text & "')"
     Else
        MsgBox "Este Tamaño de Equipo ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_tamano_equip SET tama_equip_descripcion = '" & Trim$(txt_desc.Text) & "', " & _
                               "          tama_equip_abrev       = '" & Txt_abrev.Text & "' " & _
                               " WHERE tama_equip_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and tama_equip_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_tamano_equip set tama_equip_status = 'E' " & _
             " WHERE tama_equip_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and tama_equip_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
'    CReport.WindowTitle = App.Title
    'CReport.Action = 1
    Call Excel_repo_mant("tamano_equipo")
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
End Sub
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select tama_equip_linea as linea, tama_equip_codigo as codigo, tama_equip_descripcion as descripcion, tama_equip_abrev as Abrev, tama_equip_status  as Status from docu_tamano_equip ORDER BY tama_equip_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_tamano_equip set tama_equip_status='A' " & _
                  "Where tama_equip_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And tama_equip_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("tama_equip_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("tama_equip_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("tama_equip_descripcion"))
   Txt_abrev.Text = IIf(Not IsNull(RDC_dato.Recordset.Fields("tama_equip_abrev")), RDC_dato.Recordset.Fields("tama_equip_abrev"), "")
...
sql = "Select * FROM docu_tamano_equip WHERE tama_equip_linea ='" & Cmb_linea.BoundText & "' and tama_equip_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_tamano_equip WHERE tama_equip_linea ='" & Cmb_linea.BoundText & "' and tama_equip_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
