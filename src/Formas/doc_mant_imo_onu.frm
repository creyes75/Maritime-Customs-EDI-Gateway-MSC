...
Begin VB.Form doc_mant_imo_onu 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de IMO ONU(Peligorsidad)"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_imo_onu.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_imo_onu.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_imo_onu.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_imo_onu.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   195
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_imo_onu"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT imo_onu_codigo FROM docu_imo_onu WHERE imo_onu_linea ='" & Cmb_linea.BoundText & "' and  imo_onu_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_imo_onu (imo_onu_linea, imo_onu_codigo, imo_onu_descripcion, imo_onu_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Este código de Peligrosidad IMO ONU ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_imo_onu SET imo_onu_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE imo_onu_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and imo_onu_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_imo_onu set imo_onu_status = 'E' " & _
             " WHERE imo_onu_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and imo_onu_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select imo_onu_linea as linea, imo_onu_codigo as codigo, imo_onu_descripcion as descripcion, imo_onu_status  as Status from docu_imo_onu ORDER BY imo_onu_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_imo_onu set imo_onu_status='A' " & _
                  "Where imo_onu_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And imo_onu_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("imo_onu_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("imo_onu_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("imo_onu_descripcion"))
...
sql = "Select * FROM docu_imo_onu WHERE imo_onu_linea ='" & Cmb_linea.BoundText & "' and imo_onu_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_imo_onu WHERE imo_onu_linea ='" & Cmb_linea.BoundText & "' and imo_onu_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
