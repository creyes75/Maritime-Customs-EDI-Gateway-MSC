...
Begin VB.Form doc_expo_copia 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Copia de Bl's de Exportación"
   ClientHeight    =   4275
   ClientLeft      =   4230
   ClientTop       =   2640
   ClientWidth     =   6060
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Frame Fra_pto_descarga 
         ...
         Top             =   735
         Width           =   2205
         Begin MSAdodcLib.Adodc data_pto_des_new 
            Height          =   330
            Left            =   1470
            Top             =   210
            Visible         =   0   'False
            ...
            _Version        =   393216
         End
         Begin MSDataListLib.DataCombo cmb_pto_des_new 
            Height          =   315
            Left            =   105
            TabIndex        =   7
            Top             =   225
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame7 
         ...
         Top             =   765
         Width           =   1500
         Begin MSDataListLib.DataCombo Cmb_pto_des 
            Height          =   315
            Left            =   105
            TabIndex        =   3
            Top             =   240
            ...
            Text            =   ""
         End
         Begin MSAdodcLib.Adodc data_pto_des 
            Height          =   330
            Left            =   480
            Top             =   240
            Visible         =   0   'False
   ...
   Begin Threed.SSCommand btn_sgte 
      ...
      _ExtentY        =   873
      _StockProps     =   78
      Picture         =   "doc_copia_expo.frx":0000
   End
   Begin Threed.SSCommand btn_elim 
      ...
      BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
         ...
         Strikethrough   =   0   'False
      EndProperty
      Picture         =   "doc_copia_expo.frx":0452
   End
End
Attribute VB_Name = "doc_expo_copia"
...
If Not Cmb_buque.MatchedWithList Or Not Cmb_viaje.MatchedWithList Or _
   Not Cmb_Bl.MatchedWithList Or Not Cmb_pto_des.MatchedWithList Then
...
If Not Cmb_buque_new.MatchedWithList Or Not Cmb_viaje_new.MatchedWithList Or _
   Len(Trim(txt_bl_new.Text)) = 0 Or Not cmb_pto_des_new.MatchedWithList Then
...
' se valida que el bl destino no exista
   sql = "SELECT * FROM docu_blcab_exp " & _
         ...
         "   and blcab_puer_des = '" & puerto_new & "'" & _
...
If Not reg2.EOF Then
   ...
   puerto_emb = reg2("blcab_puer_emb")
   
   sql = "INSERT INTO docu_blcab_exp SELECT blcab_linea,   '" & buque_new & "', '" & viaje_new & "', " & _
                                     " blcab_puer_emb, '" & puerto_new & "',   blcab_emp_resp, '" & _
   ...
   sql = "insert into docu_bldet_exp " & _
         "SELECT bldet_linea,   '" & buque_new & "', '" & viaje_new & "', " & _
               " bldet_puer_emb,'" & puerto_new & "',     bldet_emp_resp, '" & _
               ...
               " bldet_tip_equi,     bldet_cond_cont,     bldet_danger_cargo, " & _
               " bldet_un_number,    bldet_imdg_page " & _
         " FROM docu_bldet_exp WHERE bldet_linea ='" & linea & "'"
   sql = sql + " and bldet_buque    = '" & buque & "'"
   sql = sql + " and bldet_viaje    = '" & viaje & "'"
   sql = sql + " and bldet_puer_des = '" & puerto & "'"
   sql = sql + " and bldet_puer_emb = '" & puerto_emb & "'"
   ...
   sql = "INSERT INTO docu_cont_exp  " & _
         "SELECT cont_linea,    '" & buque_new & "', '" & viaje_new & "', " & _
                "cont_puer_emb, '" & puerto_new & "',    cont_contenedor,  " & _
   ...
   sql = sql & "  FROM docu_cont_exp, docu_bldet_exp " & _
               ...
               "        and cont_puer_des = '" & puerto_new & "'" & _
               "        and cont_puer_emb = '" & puerto_emb & "')"
   Gcon_docu.Execute sql, no_cont
   
   ' **************************************************************************
   '    llena los datos de fletes
   '***************************************************************************
   sql = "insert into docu_blflete_exp " & _
         "SELECT flete_linea,   '" & buque_new & "', '" & viaje_new & "', " & _
               " flete_puer_emb,'" & puerto_new & "',     flete_emp_resp, '" & _
                 bl_new & "',        flete_secuencia,     flete_cargo, " & _
               " flete_debtor,       flete_valor,         flete_signo, " & _
               " flete_moneda,       flete_marcas,        flete_lugar, " & _
               " flete_imp_bl,       flete_valor_unit,    flete_por, " & _
               " flete_cantidad " & _
         " FROM docu_blflete_exp WHERE flete_linea ='" & linea & "'"
   sql = sql + " and flete_buque    = '" & buque & "'"
   sql = sql + " and flete_viaje    = '" & viaje & "'"
   sql = sql + " and flete_puer_des = '" & puerto & "'"
   sql = sql + " and flete_puer_emb = '" & puerto_emb & "'"
   sql = sql + " and flete_cono_emb = '" & bl & "'"
   Gcon_docu.Execute sql, no_flete
   
   '***************************************************************************
   Call actualiza_totales_manifiesto_expo(Cmb_linea.BoundText, Cmb_buque_new.BoundText, Cmb_viaje_new.BoundText, puerto_emb, cmb_pto_des_new.BoundText, reg2("blcab_emp_resp"))
   ...
   If Chk_borra_bl.Value = 1 Then
        sql = "Delete " & _
                    ...
                    "           and bldet_puer_des = '" & puerto & "'" & _
                    "           and bldet_puer_emb = '" & puerto_emb & "'" & _
                    "           and bldet_cono_emb = '" & bl & "')" & _
                    "   and cont_contenedor not in (" & _
                    "       Select bldet_contenedor FROM docu_bldet_exp " & _
                    "       WHERE bldet_linea      = '" & linea & "'" & _
        ...
        sql = "Delete FROM docu_blflete_exp " & _
                    ...
                    "   and flete_puer_des = '" & puerto & "'" & _
                    "   and flete_puer_emb = '" & puerto_emb & "'" & _
        ...
        Call actualiza_totales_manifiesto_expo(Cmb_linea.BoundText, Cmb_buque.BoundText, Cmb_viaje.BoundText, puerto_emb, Cmb_pto_des.BoundText, reg2("blcab_emp_resp"))
   End If ' fin de borrado de bl original
   Gcon_docu.CommitTrans
   msg = "Se Copiaron exitosamente: " & Chr(13) & _
               " - Bls          = " & no_cab & Chr(13) & _
               " - detalles     = " & no_det & Chr(13) & _
               " - contenedores = " & no_cont & Chr(13) & _
               " - fletes       = " & no_flete
   If Chk_borra_bl.Value = 1 Then
        msg = msg & Chr(13) & " y se Eliminaron : " & Chr(13) & _
                              " - Bls          = " & no_cab_del & Chr(13) & _
                              " - detalles     = " & no_det_del & Chr(13) & _
                              " - contenedores = " & no_cont_del & Chr(13) & _
                              " - fletes       = " & no_flete_del
   End If
...
Private Sub Cmb_pto_des_Change()
   Dim sql As String
   sql = "Select * from docu_blcab_exp " & _
         " Where blcab_linea    ='" & Cmb_linea.BoundText & "'" & _
         "   And blcab_buque    ='" & Cmb_buque.BoundText & "'" & _
         "   And blcab_viaje    ='" & Cmb_viaje.BoundText & "'" & _
         "   And blcab_puer_des ='" & Cmb_pto_des.BoundText & "'" & _
         "   And blcab_puer_emb ='" & Gstr_param.pto_descarga & "'" '& _
...
Private Sub Cmb_pto_des_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then SendKeys "{TAB}"
End Sub


Private Sub cmb_pto_des_new_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then SendKeys "{TAB}"
End Sub
...
carga_datacombo data_pto_des, Cmb_pto_des, sql, "mani_puer_des", "mani_puer_des"
...
Private Sub Cmb_viaje_new_Change()
   ...
   carga_datacombo data_pto_des_new, cmb_pto_des_new, sql, "mani_puer_des", "mani_puer_des"
End Sub
