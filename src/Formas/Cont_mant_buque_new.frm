...
Begin VB.Form doc_mant_buque 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Buque"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "Cont_mant_buque_new.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "Cont_mant_buque_new.frx":030A
      ...
      TabPicture(1)   =   "Cont_mant_buque_new.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "Cont_mant_buque_new.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   17
            Top             =   195
            Width           =   465
      ...
      Begin Threed.SSFrame fra_tipo_rol 
         ...
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            ...
            Strikethrough   =   0   'False
         EndProperty
         Begin VB.CheckBox Chk_buque_prueba 
            Caption         =   "Buque de Prueba"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
   ...
   End
End
Attribute VB_Name = "doc_mant_buque"
 ...
 '     Valida_Datos = False
  '    If txt_codigo.Enabled Then txt_codigo.SetFocus
   ...
   ElseIf Trim$(txt_desc.Text) = "" Then
      ...
      'Exit Function
    ElseIf Trim$(Txt_nacio.Text) = "" Then
      MsgBox "Ingrese la Nacionalidad del Buque", vbInformation, App.Title
      Txt_nacio.SetFocus
      bandera = False
   Else
   bandera = True
...
'   End If
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT buqu_codigo, buqu_status FROM docu_buque WHERE buqu_linea ='" & Cmb_linea.BoundText & "' and  buqu_codigo = '" & txt_codigo.Text & "'"
     'Set REG = GCON_DOCU.OpenRecordset(sql, rdOpenKeyset)
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_buque (buqu_linea, buqu_codigo, buqu_descripcion, buqu_nacio, buqu_status, buqu_status_prueba)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "','" & Trim$(Txt_nacio.Text) & "','A', '" & IIf(Chk_buque_prueba.Value, "S", "N") & "')"
     Else
        If reg(1) = "E" Then
            resp = MsgBox("Este buque ya existe pero fue eliminado, desea reactivar este codigo? ", vbQuestion + vbYesNo, App.Title)
            If resp = vbYes Then
                sql = "UPDATE docu_buque SET buqu_status   = 'A'" & _
                      " WHERE buqu_linea  = '" & Cmb_linea.BoundText & "'" & _
                      "   and buqu_codigo = '" & Trim$(txt_codigo.Text) & "'"
                Gcon_docu.Execute sql
                MsgBox "Se reactivo este buque por favor, entre por la opcion de modificacion", vbExclamation, App.Title
            End If
        Else
            MsgBox "Este Buque ya existe, entre por la opcion modificación", vbExclamation, App.Title
        End If
        Gcon_docu.CommitTrans
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_buque SET buqu_descripcion = '" & Trim$(txt_desc.Text) & "', " & _
                               "buqu_status   = 'A'" & _
                               ",buqu_nacio ='" & Trim$(Txt_nacio.Text) & "'" & _
                               ",buqu_status_prueba = '" & IIf(Chk_buque_prueba.Value, "S", "N") & "'" & _
                               " WHERE buqu_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and buqu_codigo = '" & Trim$(txt_codigo.Text) & "'"
...
'      End If
      ...
      sql = "UPDATE docu_buque SET buqu_status ='E' " & _
            " WHERE buqu_linea  = '" & Cmb_linea.BoundText & "'" & _
            "   and buqu_codigo = '" & txt_codigo.Text & "'"
   End If
   Gcon_docu.Execute sql
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select buqu_linea as linea, buqu_codigo as codigo, buqu_descripcion as descripcion, buqu_status as Status, buqu_nacio, buqu_status_prueba from docu_buque Where buqu_status = 'A'"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   Grid_dato.Refresh
   fra_tipo_rol.Enabled = False
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("buqu_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("buqu_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("buqu_descripcion"))
   Txt_nacio.Text = Trim$(RDC_dato.Recordset.Fields("buqu_nacio"))
   Chk_buque_prueba.Value = IIf(Trim$(RDC_dato.Recordset.Fields("buqu_status_prueba")) = "S", 1, 0)
   'Txt_nacio.Text = IIf(Not IsNull(RDC_dato.Recordset.Fields("buqu_status")), RDC_dato.Recordset.Fields("buqu_status"), "")
...
Public Sub txt_codigo_LostFocus()
Dim reg     As New ADODB.Recordset
Dim sql     As String
Dim i       As Integer
...
sql = "Select * FROM docu_buque WHERE buqu_linea ='" & Cmb_linea.BoundText & "' and buqu_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_buque WHERE buqu_linea ='" & Cmb_linea.BoundText & "' and buqu_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
...
Private Sub Txt_nacio_KeyPress(KeyAscii As Integer)
If KeyAscii = 13 Then SendKeys "{TAB}"
KeyAscii = Asc(UCase(Chr(KeyAscii)))
End Sub
