...
Begin VB.Form doc_mant_emba 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Embalaje"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_embalaje.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_embalaje.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_embalaje.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_embalaje.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   195
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_emba"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT emba_codigo FROM docu_embalaje WHERE emba_linea ='" & Cmb_linea.BoundText & "' and  emba_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_embalaje (emba_linea, emba_codigo, emba_descripcion, emba_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Este Embalaje ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_embalaje SET emba_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE emba_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and emba_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_embalaje set emba_status = 'E' " & _
             " WHERE emba_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and emba_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select emba_linea as linea, emba_codigo as codigo, emba_descripcion as descripcion, emba_status  as Status from docu_embalaje ORDER BY emba_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_embalaje set emba_status='A' " & _
                  "Where emba_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And emba_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("emba_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("emba_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("emba_descripcion"))
...
sql = "Select * FROM docu_embalaje WHERE emba_linea ='" & Cmb_linea.BoundText & "' and emba_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_embalaje WHERE emba_linea ='" & Cmb_linea.BoundText & "' and emba_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
