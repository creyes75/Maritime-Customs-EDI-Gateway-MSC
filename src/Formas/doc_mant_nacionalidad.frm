...
Begin VB.Form doc_mant_nacionalidad 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Nacionalidad"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_nacionalidad.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_nacionalidad.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_nacionalidad.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_nacionalidad.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   13
            Top             =   195
            Width           =   465
   ...
   End
End
Attribute VB_Name = "doc_mant_nacionalidad"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT naci_codigo FROM docu_nacionalidad WHERE naci_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_nacionalidad ( naci_codigo, naci_descripcion, naci_status)" & _
             " VALUES ('" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Esta Nacionalidad ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_nacionalidad SET naci_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE " & _
                               "   naci_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_nacionalidad set naci_status = 'E' " & _
             " WHERE " & _
             "   naci_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
'    CReport.WindowTitle = App.Title
    'CReport.Action = 1
    Call Excel_repo_mant("nacionalidad")
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
End Sub
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select naci_codigo as codigo, naci_descripcion as descripcion, naci_status  as Status from docu_nacionalidad ORDER BY naci_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_nacionalidad set naci_status='A' " & _
                  "Where " & _
                  "  naci_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("naci_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("naci_descripcion"))
...
sql = "Select * FROM docu_nacionalidad WHERE naci_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_nacionalidad WHERE naci_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
