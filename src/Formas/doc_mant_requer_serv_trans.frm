...
Begin VB.Form doc_mant_requer_serv_trans 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Requerimiento de Servicio de Transporte"
   ClientHeight    =   3870
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6480
   Icon            =   "doc_mant_requer_serv_trans.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   3870
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_requer_serv_trans.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_requer_serv_trans.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_requer_serv_trans.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   15
            Top             =   195
            Width           =   465
      ...
      Begin Threed.SSFrame fra_tipo_rol 
         ...
         _ExtentY        =   3678
         _StockProps     =   14
         Caption         =   " Datos de Req. serv. transp"
         ForeColor       =   128
   ...
   End
End
Attribute VB_Name = "doc_mant_requer_serv_trans"
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT requ_serv_codigo FROM docu_requer_serv_trans WHERE requ_serv_linea ='" & Cmb_linea.BoundText & "' and  requ_serv_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_requer_serv_trans (requ_serv_linea, requ_serv_codigo, requ_serv_descripcion, requ_serv_status)" & _
             " VALUES ('" & Cmb_linea.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "', 'A')"
     Else
        MsgBox "Esta Requerimiento de Servicio de Transporte ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_requer_serv_trans SET requ_serv_descripcion = '" & Trim$(txt_desc.Text) & "' " & _
                               " WHERE requ_serv_linea  = '" & Cmb_linea.BoundText & "'" & _
                               "   and requ_serv_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_requer_serv_trans set requ_serv_status = 'E' " & _
             " WHERE requ_serv_linea  = '" & Cmb_linea.BoundText & "'" & _
             "   and requ_serv_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
'    CReport.WindowTitle = App.Title
    'CReport.Action = 1
    Call Excel_repo_mant("req_serv_transp")
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
End Sub
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select requ_serv_linea as linea, requ_serv_codigo as codigo, requ_serv_descripcion as descripcion, requ_serv_status  as Status from docu_requer_serv_trans ORDER BY requ_serv_descripcion"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
   ...
   If Not RDC_grid_dato.Recordset.EOF Then
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_requer_serv_trans set requ_serv_status='A' " & _
                  "Where requ_serv_linea  ='" & RDC_grid_dato.Recordset("linea") & "'" & _
                  "  And requ_serv_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_linea.BoundText = Trim$(RDC_dato.Recordset.Fields("requ_serv_linea"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("requ_serv_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("requ_serv_descripcion"))
...
sql = "Select * FROM docu_requer_serv_trans WHERE requ_serv_linea ='" & Cmb_linea.BoundText & "' and requ_serv_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_requer_serv_trans WHERE requ_serv_linea ='" & Cmb_linea.BoundText & "' and requ_serv_descripcion like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
