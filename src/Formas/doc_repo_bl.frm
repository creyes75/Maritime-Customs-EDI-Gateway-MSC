...
'    If MsgBox("Este proceso tomará algunos minutos. Desea continuar?", vbQuestion + vbYesNo) = vbNo Then Exit Sub
    ...
    '****************************************************************************
       ...
       oCell.Range.Text = Mid(reg("blcab_direc_consig"), 1, 61)
       ...
       'End If
       ' se buscan la cantidad de contenedores por tipo
       sql = "Select count(*), tama_equip_abrev " & _
               " from docu_bldet, docu_cont, docu_tamano_equip " & _
       ...
       If Not REG_det.EOF Then
          oCell.Range.Text = reg("blcab_bultos_tot") & " " & REG_det("bldet_embalaje") & no_cont ' nuevo se agrega el embalaje    16/sep/2002 ' se agrega el numero de cont x tipo (oct 6 2005)
       ...
       'Set oCell = oWord.ActiveDocument.Tables(1).Cell(Row:=i, Column:=26) ' desc. carga ' ----------  SEPT 22 2008  -----------------
       'oCell.Range.Text = chk_str(desc_carga) ' ----------  SEPT 22 2008  -----------------
...
Dim ls_desc As String
    ...
    oWord.ActiveDocument.MailMerge.OpenDataSource Name:= _
        doc_origen, ConfirmConversions:=False, _
    ...
    With oMerge
         ...
         .Execute
         oWord.ActiveDocument.SaveAs FileName:=NomArchivo
         'oWord.ActiveDocument.Tables(3).heigth
