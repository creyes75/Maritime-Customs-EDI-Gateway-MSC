...
Begin VB.Form doc_mant_consig 
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Mantenimiento de Importador/Exportador"
   ClientHeight    =   4230
   ClientLeft      =   3360
   ClientTop       =   4110
   ClientWidth     =   6540
   Icon            =   "doc_mant_consig.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MDIChild        =   -1  'True
   ScaleHeight     =   4230
   ...
   Begin TabDlg.SSTab Tab_dato 
      ...
      TabPicture(0)   =   "doc_mant_consig.frx":030A
      ...
      TabPicture(1)   =   "doc_mant_consig.frx":0326
      ...
      Begin VB.Frame Frame1 
         ...
         Begin VB.CommandButton Cmd_buscar 
            Height          =   465
            Left            =   5085
            Picture         =   "doc_mant_consig.frx":0342
            Style           =   1  'Graphical
            TabIndex        =   8
            Top             =   180
            Width           =   465
      ...
      Begin Threed.SSFrame fra_tipo_rol 
         ...
         _ExtentY        =   4630
         _StockProps     =   14
         Caption         =   " Datos de Tipo de carga"
         ForeColor       =   128
         ...
         Begin VB.CommandButton Cmd_hlp_nac_consignatario 
            ...
            Height          =   300
            Left            =   5205
            Picture         =   "doc_mant_consig.frx":064C
            Style           =   1  'Graphical
            TabIndex        =   13
            ToolTipText     =   "Buscar"
            Top             =   780
         ...
         Begin VB.TextBox txt_codigo 
            ...
            Width           =   1485
         End
         Begin MSDataListLib.DataCombo Cmb_tipo 
            Height          =   315
            Left            =   1680
            TabIndex        =   10
            Top             =   435
            ...
            Text            =   ""
         End
         Begin MSAdodcLib.Adodc data_tipo 
            Height          =   330
            Left            =   3120
            Top             =   360
            Visible         =   0   'False
   ...
   End
End
Attribute VB_Name = "doc_mant_consig"
...
Public Sub habilita(ByVal opc As Boolean)
   If opc Then
      ...
      Else
         Cmb_tipo.Enabled = opc
         txt_codigo.Enabled = opc
         txt_desc.Enabled = opc
         Txt_nac_consignatario.Enabled = opc
         txt_direcc.Enabled = opc
         txt_fono.Enabled = opc
         txt_fax.Enabled = opc
   ...
   Else
      ...
      Txt_nac_consignatario.Enabled = True
...
valida_Datos = True
 ...
 If Not Cmb_tipo.MatchedWithList Then
      valida_Datos = False
      MsgBox "Ingrese el Tipo", vbInformation, App.Title
      If Cmb_tipo.Enabled Then Cmb_tipo.SetFocus
      Exit Function
 ElseIf Trim$(txt_codigo.Text) = "" Then
      MsgBox "Ingrese el Código del Importador/Exportador", vbInformation, App.Title
      valida_Datos = False
      If txt_codigo.Enabled Then txt_codigo.SetFocus
      Exit Function
 ElseIf Trim$(txt_desc.Text) = "" Then
      MsgBox "Ingrese el Nombre del Importador/Exportador", vbInformation, App.Title
      txt_desc.SetFocus
      valida_Datos = False
...
Private Sub Graba_trol()
   ...
   If Lstr_opc = "A" Then
     sql = "SELECT consig_codigo FROM docu_consignatario WHERE consig_tipo_id ='" & Cmb_tipo.BoundText & "' and  consig_codigo = '" & txt_codigo.Text & "'"
     reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
     If reg.EOF Then
        sql = "INSERT INTO docu_consignatario (consig_tipo_id, consig_codigo, consig_nombre,consig_nacional, consig_direccion, consig_fono, consig_fax, consig_mail, consig_status)" & _
             " VALUES ('" & Cmb_tipo.BoundText & "', '" & Trim$(txt_codigo.Text) & "','" & Trim$(txt_desc.Text) & "','" & Trim(Me.Txt_nac_consignatario.Text) & "', '" & Trim(txt_direcc.Text) & "','" & Trim(txt_fono.Text) & "', '" & Trim(txt_fax.Text) & "', '" & Trim(txt_mail.Text) & "', 'A')"
     Else
        MsgBox "Este Importador / Exportador ya existe, entre por la opcion modificación", vbExclamation, App.Title
        Exit Sub
     End If
   ElseIf Lstr_opc = "M" Then
      sql = "UPDATE docu_consignatario SET consig_nombre = '" & Trim$(txt_desc.Text) & "', " & _
                               " consig_nacional ='" & Txt_nac_consignatario.Text & "', " & _
                               " consig_direccion = '" & txt_direcc.Text & "', " & _
                               " consig_fono      = '" & txt_fono.Text & "', " & _
                               " consig_fax       = '" & txt_fax.Text & "',  " & _
                               " consig_mail      = '" & txt_mail.Text & "' " & _
                               " WHERE consig_tipo_id= '" & Cmb_tipo.BoundText & "'" & _
                               "   and consig_codigo = '" & Trim$(txt_codigo.Text) & "'"
   ElseIf Lstr_opc = "E" Then
       sql = "UPDATE docu_consignatario set consig_status = 'E' " & _
             " WHERE consig_tipo_id= '" & Cmb_tipo.BoundText & "'" & _
             "   and consig_codigo = '" & Trim$(txt_codigo.Text) & "'"
   End If
...
Select Case cmd_agregar.Caption
  ...
  carga_datacombo Me.data_tipo, Cmb_tipo, "Select * from docu_tipo_id", "tipo_id_codigo", "tipo_id_descripcion"
'   data_tipo.ConnectionString = Gstr_conex_docu
'   data_tipo.RecordSource = "Select * from docu_tipo_nav where linea_status ='A'"
'   data_tipo.Refresh
'   Cmb_tipo.BoundColumn = "linea_codigo"
'   Cmb_tipo.ListField = "linea_descripcion"
'   Set Cmb_tipo.RowSource = data_tipo
   Cmb_tipo.SetFocus
...
Private Sub Cmd_buscar_Click()
    ...
    Else
        RDC_grid_dato.Recordset.Find "nombre like '" & Txt_crite.Text & "*'"
    End If
...
'    CReport.WindowTitle = App.Title
    'CReport.Action = 1
      Call Excel_repo_mant("consignatario")
    Exit Sub
Errores:
    MsgBox "Ocurrio el error " & Err.Description, vbInformation, App.Title
End Sub
...
Private Sub Form_Load()
   ...
   RDC_grid_dato.RecordSource = "Select consig_tipo_id as tipo, consig_codigo as codigo, consig_nombre as nombre, consig_nacional as nacionalidad, consig_direccion as direccion, consig_fono as fono, consig_fax as fax, consig_mail as mail, consig_status  as Status from docu_consignatario ORDER BY consig_nombre"
   RDC_grid_dato.Refresh
   Set Grid_dato.DataSource = RDC_grid_dato
...
Private Sub Grid_dato_DblClick()
   If Not RDC_grid_dato.Recordset.EOF Then
      Cmb_tipo.BoundText = RDC_grid_dato.Recordset("tipo")
      ...
      If RDC_grid_dato.Recordset("status") = "E" Then
         If MsgBox("Este Registro esta con STATUS Eliminado, desea Activarlo?", vbQuestion + vbYesNo) = vbYes Then
            sql = "Update docu_consignatario set consig_status='A' " & _
                  "Where consig_tipo_id  ='" & RDC_grid_dato.Recordset("tipo") & "'" & _
                  "  And consig_codigo ='" & RDC_grid_dato.Recordset("codigo") & "'"
            Gcon_docu.Execute sql
         End If
      End If
...
If Not RDC_dato.Recordset.EOF Then
   Cmb_tipo.BoundText = Trim$(RDC_dato.Recordset.Fields("consig_tipo_id"))
   txt_codigo.Text = Trim$(RDC_dato.Recordset.Fields("consig_codigo"))
   txt_desc.Text = Trim$(RDC_dato.Recordset.Fields("consig_nombre"))
   Txt_nac_consignatario.Text = Trim$(RDC_dato.Recordset.Fields("consig_nacional"))
   txt_direcc.Text = Trim$(RDC_dato.Recordset.Fields("consig_direccion"))
   txt_fono.Text = Trim$(RDC_dato.Recordset.Fields("consig_fono"))
   txt_fax.Text = Trim$(RDC_dato.Recordset.Fields("consig_fax"))
   txt_mail.Text = Trim$(RDC_dato.Recordset.Fields("consig_mail"))
...
sql = "Select * FROM docu_consignatario WHERE consig_tipo_id ='" & Cmb_tipo.BoundText & "' and consig_codigo like '" & txt_codigo.Text & "'"
...
sql = "Select * FROM docu_consignatario WHERE consig_tipo_id ='" & Cmb_tipo.BoundText & "' and consig_nombre like '" & txt_desc.Text & "'"
reg.Open sql, Gcon_docu, adOpenDynamic, adLockOptimistic
'Set REG = GCON_DOCU.OpenRecordset(sql)
If Not reg.EOF Then
   RDC_dato.RecordSource = sql
